...
def update_state(raw_state: str, lines: list[str]) -> str:
    ...
    current_visible_lines = [item.line for item in state if item.count % 2]
    ...
    for i in range(len(status_lines)+1):
        if i == len(status_lines) or (status_lines[i].on_left and
            ...
            for j in range(begin, i):
                ...
                in_child = status_line.count % 2
                if status_line.on_left != status_line.on_right:
                    if in_child == status_line.on_left:
                        hit_left = True
                    else:
                        hit_right = True
                if in_child and status_line.on_left != status_line.on_right:
                    found_add = True
            if hit_left and hit_right and found_add:
                for j in range(begin, i):
                    ...
                    if status_line.on_left or status_line.on_right:
                        result_lines.append((_unwrap(status_line.line), conflict_code(in_child, status_line.on_left, status_line.on_right)))
            else:
                for j in range(begin, i):
                    status_line = status_lines[j]
...
def merge_trees(output: Output, tree1: Tree, tree2: Tree, anchored_right: bool) -> None:
    ...
    if tree1.line is not None:
        output.append(OutputItem(
            ...
            tree1.depth,
            anchored_right,
            max(tree1.count, tree2.count),
            tree1.count % 2,
            tree2.count % 2,
        ))
    merge_tree_lists(output, tree1.high_trees, tree2.high_trees, False)

def merge_tree_lists(output: Output, left_trees: list[Tree], right_trees: list[Tree], anchored_right: bool) -> None:
    pos1 = 0
    pos2 = 0
...
def insert_tree(output: Output, tree: Tree, from_right: bool, anchored_right: bool) -> None:
