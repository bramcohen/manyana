# Pre-DAG transient rebase implementation, plus the test suite that exercises it.
#
# Kept short-term so test_rebase_jam_matches_old_rebase_visible_lines (at the
# bottom) can prove the DAG-aware rebase in rebase.py preserves identical
# semantics.  Drop this whole file after merge — the transient API has no
# callers outside this file.

from manyana import (
    initial_state, update_state, merge_states, current_lines,
    DAG, BEGIN_PREFIX, DIVIDER_PREFIX, END,
)
from rebase import rebase_jam   # for the equivalence test only


# ---------------------------------------------------------------------------
# Transient (pre-DAG) rebase implementation
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Equivalence: transient rebase vs DAG-aware rebase_jam
# ---------------------------------------------------------------------------
#
# This is the bridge that justifies keeping the transient implementation
# alongside the DAG one: across a battery of scenarios, both paths must
# produce identical visible content.

def _run_both_paths(initial, local_edits, main_edits):
    """Apply the same scenario through DAG-aware and transient paths.
    Returns (dag_visible_lines, transient_visible_lines)."""
    # DAG path
    d = DAG()
    r = d.root(initial)
    parent = r
    local_ids = []
    for edit in local_edits:
        parent = d.commit(parent, edit)
        local_ids.append(parent)
    main_id = r
    for edit in main_edits:
        main_id = d.commit(main_id, edit)
    final, _, _ = rebase_jam(d, local_ids, main_id)
    dag_lines = current_lines(d.state(final))
    ...
    local_states = []
    for edit in local_edits:
        s = update_state(s, edit)
        local_states.append(s)
    main_state = base
    for edit in main_edits:
        main_state = update_state(main_state, edit)
    o_final, _ = rebase(local_states, main_state)
    transient_lines = current_lines(o_final)

    return dag_lines, transient_lines


# (name, initial_lines, local_edits, main_edits)
_EQUIVALENCE_SCENARIOS = [
    ('two-commit insert + main advances',
        ['A', 'B', 'C'],
        [['A', 'X', 'B', 'C'], ['A', 'X', 'B', 'Y', 'C']],
        [['A', 'B', 'C', 'Z']]),
    ('single commit, no main advance',
        ['A', 'B'],
        [['A', 'X', 'B']],
        []),
    ('single commit, main also advances',
        ['A', 'B', 'C'],
        [['A', 'X', 'B', 'C']],
        [['A', 'B', 'C', 'Z']]),
    ('local deletes, main inserts',
        ['A', 'B', 'C', 'D'],
        [['A', 'C', 'D'], ['A', 'C']],
        [['A', 'B', 'C', 'D', 'Z']]),
    ('conflicting inserts at same location',
        ['A', 'B'],
        [['A', 'X', 'B']],
        [['A', 'Y', 'B']]),
    ('main deletes, local inserts',
        ['A', 'B', 'C'],
        [['A', 'X', 'B', 'C']],
        [['A', 'C']]),
    ('multi-step main advance with deletions',
        ['A', 'B', 'C'],
        [['A', 'X', 'B', 'C']],
        [['A', 'B', 'C', 'M1'], ['A', 'C', 'M1']]),
    ('add → delete → resurrect locally',
        ['A'],
        [['A', 'X'], ['A'], ['A', 'X']],
        [['A', 'Z']]),
    ('empty initial, both sides add',
        [],
        [['hello']],
        [['world']]),
]
...
def test_rebase_jam_matches_old_rebase_visible_lines():
    """rebase_jam (DAG-based) and rebase (transient) produce the same
    visible final content across a battery of scenarios — gives empirical
    evidence the DAG wrapper preserves semantics."""
    for name, initial, local_edits, main_edits in _EQUIVALENCE_SCENARIOS:
        dag_lines, transient_lines = _run_both_paths(initial, local_edits, main_edits)
        assert dag_lines == transient_lines, \
            f"Scenario '{name}': DAG={dag_lines}, transient={transient_lines}"
